...
homebrew-tools has adopted this security disclosure and response policy to ensure we responsibly handle critical
issues.
...
The homebrew-tools project maintains release branches for the three most recent minor releases. Applicable fixes,
...
reported to homebrew-tools privately, to minimize attacks against current users of homebrew-tools before they are fixed.
...
If you know of a publicly disclosed security vulnerability for homebrew-tools, please **IMMEDIATELY** contact us via any
...
the details of the vulnerability. The message will be fielded by the our team, which is made up of homebrew-tools
...
Use [GitHub Issues](https://github.com/Serpentiel/homebrew-tools/issues/new/choose) instead.
...
- Description of the effects of the vulnerability on homebrew-tools and the related hardware and software
  configurations, so that our team can reproduce it
- How the vulnerability affects homebrew-tools usage and an estimation of the attack surface, if there is one
- List other projects or dependencies that were used in conjunction with homebrew-tools to produce the vulnerability
...
- When you think homebrew-tools has a potential security vulnerability
- When you suspect a potential vulnerability but you are unsure that it impacts homebrew-tools
- When you know of or suspect a potential vulnerability on another project that is used by homebrew-tools, e.g.
  dependencies of homebrew-tools
...
8. Once the fix is confirmed, our team will patch the vulnerability in the next patch or minor release, and
   backport a patch release into all earlier supported releases. Upon release of the patched version of homebrew-tools,
   we will follow the [Public Disclosure Process](#public-disclosure-process)

### Public Disclosure Process

Our team publishes a public [advisory](https://github.com/Serpentiel/homebrew-tools/security/advisories) to the homebrew-tools
community via GitHub. In most cases, additional communication via Slack, Twitter, blog and other channels will assist in
educating homebrew-tools users and rolling out the patched release to affected users.

Our team will also publish any mitigating steps users can take until the fix can be applied to their homebrew-tools
setup.
